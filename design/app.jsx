...
const I = {
  ...
  file: (ext) => {
    const tint = { tex: 'var(--accent)', bib: '#6b8eaf', pdf: '#b86060', png: '#7aaa7a', sty: '#a87fb8' }[ext] || 'var(--muted)';
    return (
  ...
  upload: <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M8 11V3M8 3l-3 3M8 3l3 3M2.5 13.5h11" /></svg>,
  search: <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.3"><circle cx="7" cy="7" r="4.2" /><path d="m10.2 10.2 3 3" /></svg>,
  play:   <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M4 3l9 5-9 5V3z" /></svg>,
  check:  <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="m3 8 3.2 3.2L13 4.5" /></svg>,
  warn:   <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M8 2.5 14 13H2L8 2.5Z" /><path d="M8 7v3" /><circle cx="8" cy="11.5" r=".7" fill="currentColor" /></svg>,
  download: <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M8 3v8M8 11l-3-3M8 11l3-3M2.5 13.5h11" /></svg>,
  share: <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><circle cx="4" cy="8" r="1.8" /><circle cx="12" cy="4" r="1.8" /><circle cx="12" cy="12" r="1.8" /><path d="m5.5 7 5-2.5M5.5 9l5 2.5" /></svg>,
  bold:   <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M4 3h4.4c1.8 0 3 .9 3 2.4 0 1-.5 1.7-1.4 2 1.2.3 1.9 1.1 1.9 2.3 0 1.7-1.3 2.8-3.4 2.8H4V3Zm2 1.7v2.6h2c.9 0 1.4-.5 1.4-1.3 0-.8-.5-1.3-1.5-1.3H6Zm0 4.1V12h2.3c1 0 1.6-.5 1.6-1.3s-.6-1.3-1.7-1.3H6Z" /></svg>,
  italic: <svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M6.5 3h5l-.2 1H9.6l-1.8 8H10l-.2 1h-5l.2-1h1.8L8.6 4H6.7L6.5 3Z" /></svg>,
  sigma:  <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.2"><path d="M11.5 4V3h-7l3.5 5-3.5 5h7V12" /></svg>,
  list:   <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M5 4h8M5 8h8M5 12h8M3 4h.01M3 8h.01M3 12h.01" /></svg>,
  link:   <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M7 5 5.5 3.5a2.5 2.5 0 0 0-3.5 3.5L4 8.5M9 11l1.5 1.5a2.5 2.5 0 0 0 3.5-3.5L12.5 7.5M5.5 10.5l5-5" /></svg>,
  table:  <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><rect x="2.5" y="3.5" width="11" height="9" /><path d="M2.5 7h11M2.5 10h11M8 3.5v9" /></svg>,
  image:  <svg viewBox="0 0 16 16" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3"><rect x="2.5" y="3" width="11" height="10" rx="1" /><circle cx="6" cy="7" r="1.2" /><path d="M3 12 7 8l3 2.5L13 8" /></svg>,
  user:   <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.3"><circle cx="8" cy="6" r="2.5" /><path d="M3 13.5c.5-2.5 2.5-3.5 5-3.5s4.5 1 5 3.5" /></svg>,
};
  ...
  return (
    <header className="topbar" data-screen-label="TopBar">
      ...
      <nav className="crumbs">
        ...
        <span className="crumb-sep">/</span>
        <span className="crumb crumb-current">{docName}{dirty && <i className="dirty-dot" />}</span>
      </nav>

      <div className="topbar-actions">
        <button className="btn btn-ghost" title="Search documents">{I.search} <span>Search</span></button>
        <button className={"btn btn-primary" + (compiling ? ' is-compiling' : '')} onClick={onCompile}>
          {compiling ? <span className="spinner" /> : I.play}
          <span>{compiling ? 'Compiling…' : 'Recompile'}</span>
          <kbd>⌘↵</kbd>
        </button>
        <div className="btn-group">
          <button className="btn btn-ghost" title="Download">{I.download}<span>Export</span></button>
        </div>
        <button className="btn btn-ghost btn-icon" title="Share">{I.share}</button>
        <button className="btn btn-ghost btn-icon" onClick={onToggleTheme} title="Toggle theme">
          {theme === 'dark'
            ? <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.3"><circle cx="8" cy="8" r="3" /><path d="M8 1v2M8 13v2M1 8h2M13 8h2M3 3l1.4 1.4M11.6 11.6 13 13M3 13l1.4-1.4M11.6 4.4 13 3" /></svg>
            : <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.3"><path d="M13 9.5A5.5 5.5 0 0 1 6.5 3a.5.5 0 0 0-.7-.5 6 6 0 1 0 7.7 7.7.5.5 0 0 0-.5-.7Z" /></svg>}
        </button>
...
function FileNode({ node, depth = 0, onPick, activePath, path = '' }) {
  ...
  if (node.kind === 'folder') {
    return (
      <div>
        <div className="tree-row" style={{ paddingLeft: 8 + depth * 14 }} onClick={() => setOpen(!open)}>
          <span className="tree-chev">{I.chevron(open)}</span>
          <span className="tree-icon">{I.folder(open)}</span>
          <span className="tree-label">{node.name}</span>
        </div>
        {open && node.children.map((c) => (
          <FileNode key={c.name} node={c} depth={depth + 1} onPick={onPick} activePath={activePath} path={full} />
        ))}
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <span className="tree-label">{node.name}</span>
      {node.dirty && <span className="tree-dirty" title="Unsaved changes">●</span>}
      ...
      {tab === 'files' && (
        <div className="side-body">
          <div className="side-actions">
            <button className="side-btn">{I.upload}<span>Upload</span></button>
    ...
    <div className="ed-toolbar">
      {items.map((it, i) =>
        it.sep
          ...
          : <button key={i} className="ed-tool" title={it.t}>{it.i}<span>{it.l}</span></button>
      )}
      ...
      <div className="pane-head">
        <div className="pane-tabs">
          <div className="pane-tab is-active">
            {I.file('tex')}<span>main.tex</span><i className="dirty-dot" />
          </div>
          <div className="pane-tab">
            {I.file('bib')}<span>references.bib</span>
          </div>
        </div>
        <div className="pane-head-right">
          <span className="ed-pos">Ln {cursor.line}, Col {cursor.col}</span>
        </div>
      </div>
      ...
      <div className="editor-scroll">
        <div className="editor-code">
          <div className="gutter">
            {lines.map((_, i) => (
              <div key={i} className={"ln" + (i + 1 === cursor.line ? ' ln-active' : '')}>{i + 1}</div>
            ))}
          </div>
    ...
    <section className="preview-pane" data-screen-label="Preview">
      <div className="pane-head preview-head">
        ...
        <div className="prev-controls">
          ...
          <button className="ctrl-btn" onClick={() => setZoom(z => Math.min(180, z + 10))}>+</button>
      ...
      <div className="preview-scroll">
        <div className="preview-stage" style={{ transform: `scale(${zoom / 100})` }}>
          <PaperPreview />
          {compiling && <div className="prev-overlay"><span className="spinner spinner-lg" /></div>}
        </div>
      </div>
    </section>
  );
