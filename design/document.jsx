  ...
  return (
    <article className="paper">
      ...
      <section className="paper-abstract">
        ...
        <p>
          We revisit the convergence behavior of stochastic gradient
          descent (SGD) under relaxed smoothness assumptions. We prove an
          {' '}<K tex="O(1/\sqrt{T})" /> rate to a stationary point for a broad
          class of non-convex objectives and validate the bound on a suite
          of deep network training tasks.
        </p>
      </section>

      <section className="paper-section">
        <h2><span className="sec-num">1</span> Introduction</h2>
        <p>
          Let <K tex="f : \mathbb{R}^d \to \mathbb{R}" /> be a possibly
          non-convex objective with stochastic gradient oracle
          {' '}<K tex="g(x,\xi)" /> such that
        ...
        <p>
          where <K tex="\eta_t > 0" /> is the step size at iteration{' '}
          <K tex="t" />.
        </p>
      </section>

      <section className="paper-section">
        <h2><span className="sec-num">2</span> Main Result</h2>
        <div className="theorem">
          <p>
            <strong>Theorem 1 (Convergence rate).</strong>{' '}
            <em>
              Assume <K tex="f" /> is <K tex="L" />–smooth and that
              {' '}<K tex={String.raw`\mathbb{E}\|g(x,\xi) - \nabla f(x)\|^2 \le \sigma^2`} />.
              Choosing <K tex={String.raw`\eta_t = \eta / \sqrt{T}`} /> with
              {' '}<K tex={String.raw`\eta \le 1/L`} />,
            </em>
          </p>
          <K display tex={String.raw`\frac{1}{T}\sum_{t=1}^{T} \mathbb{E}\|\nabla f(x_t)\|^2 \;\le\; \frac{2(f(x_0)-f^\star)}{\eta\sqrt{T}} + \frac{L\eta\sigma^2}{\sqrt{T}}.`} />
        </div>

        <h3><span className="sec-num">2.1</span> Proof sketch</h3>
        <p>
          By <K tex="L" />–smoothness and the update rule (1),
        </p>
        <K display tex={String.raw`f(x_{t+1}) \le f(x_t) - \eta_t \langle \nabla f(x_t), g_t \rangle + \tfrac{L\eta_t^2}{2}\|g_t\|^2.`} />
        <p>
          Taking expectations and telescoping over
          {' '}<K tex="t = 1,\dots,T" /> yields the stated bound.
          <span className="qed">□</span>
        </p>
      </section>

      <section className="paper-section">
        <h2><span className="sec-num">3</span> Experiments</h2>
        <p>
          We compare against momentum SGD and Adam on CIFAR-10 with a
          ResNet-18 backbone. Figure 1 reports the training loss across
          200 epochs.
        </p>
        <figure className="paper-fig">
          ...
          <figcaption>
            <strong>Figure 1.</strong> Training loss on CIFAR-10. SGD with
            the proposed step size matches Adam after epoch 40.
          </figcaption>
        </figure>
      </section>
      ...
      <section className="paper-section paper-refs">
        <h2>References</h2>
        <ol>
          <li>Bottou, L. &amp; Bousquet, O. The tradeoffs of large scale learning. <em>NeurIPS</em>, 2008.</li>
          <li>Ghadimi, S. &amp; Lan, G. Stochastic first- and zeroth-order methods for nonconvex stochastic programming. <em>SIAM J. Optim.</em>, 2013.</li>
          <li>Kingma, D. &amp; Ba, J. Adam: A method for stochastic optimization. <em>ICLR</em>, 2015.</li>
        </ol>
      </section>
...
function FigureChart() {
  ...
  return (
    <svg viewBox="0 0 380 220" width="100%" height="100%">
      ...
      <polyline points={pts(1.5, 4.6)} fill="none" stroke="#5a6470" strokeWidth="1.4" strokeDasharray="3 3" />
      <polyline points={pts(1.45, 6.2)} fill="none" stroke="var(--accent)" strokeWidth="1.8" />
      {/* labels */}
      <text x="330" y="55"  fontSize="9" fill="var(--accent)">SGD (ours)</text>
      <text x="320" y="86"  fontSize="9" fill="#5a6470">Momentum</text>
      <text x="330" y="115" fontSize="9" fill="#9aa0a6">Adam</text>
      <text x="190" y="205" fontSize="9" fill="currentColor" textAnchor="middle">epoch</text>
      <text x="8" y="100" fontSize="9" fill="currentColor" transform="rotate(-90 8 100)" textAnchor="middle">loss</text>
    </svg>
  );
}
...
Object.assign(window, {
  PAPER_TITLE, LATEX_SOURCE, tokenizeLatex,
  PaperPreview, PROJECT_TREE, OUTLINE,
});
